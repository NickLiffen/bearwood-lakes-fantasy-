...
import { enterScoreSchema, bulkEnterScoresSchema } from './_shared/validators/scores.validator';
...
export const handler = withAdmin(async (event) => {
  ...
  try {
    ...
    if (rawBody.scores && Array.isArray(rawBody.scores)) {
      ...
      const scores = await timer.measure(
        'bulkEnterScores',
        () =>
          bulkEnterScores({
    ...
    const score = await timer.measure('enterScore', () =>
      enterScore({
        tournamentId: data.tournamentId,
